    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            ...
            if len(arg) > 2:
                ...
                FBAN = arg[1]
                REASON = " #Superfban_By_Kakashi_Userbot"
        else:
...
# Dear kanger if you remove id or username which is given below so this plugin will not work...
        if int(FBAN) == 1883752632 or int(FBAN) == 1883752632 or int(FBAN) == 1883752632 :     
            await event.edit("Sorry dear but you are trying to ban GOD....")
            return
    except BaseException:
        if FBAN == "@Its_JassManak" or FBAN == "@Its_JassManak" or FBAN == "@Its_JassManak" :
            await event.edit("Sorry Dear but you are trying to ban God....")
            return
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            ...
            return
    await event.edit(f"Kakashi UB banning you in **{len(fedList)}** for **{lightning}** .")
    try:
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**Kakashi UB unfban you in order of {lightning}**...")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
